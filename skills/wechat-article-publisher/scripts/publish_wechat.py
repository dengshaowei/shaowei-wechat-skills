...
def _number_box_style() -> str:
    return (
        "margin:0.6em auto 1.4em;padding:0.58em 1.1em;max-width:9.5em;"
        "text-align:center;background:#fff7e6;border:1px solid #ffe1a6;"
        "border-radius:10px;line-height:1.5;font-size:15px;font-weight:700;"
        "color:#9a3412;letter-spacing:0.02em;box-shadow:0 2px 8px rgba(251,146,60,0.08);"
    )


def _is_issue_number(text: str) -> bool:
    return bool(re.fullmatch(r"No\.\s*\d+", (text or "").strip(), re.I))


def _normalize_issue_number_text(text: str) -> str:
    match = re.search(r"No\.\s*(\d+)", text or "", re.I)
    return f"No.{match.group(1)}" if match else (text or "").strip()


def _make_issue_number_box(soup: BeautifulSoup, text: str):
    box = soup.new_tag("section")
    box["style"] = _number_box_style()
    box.string = _normalize_issue_number_text(text)
    return box


def _render_issue_number_boxes(soup: BeautifulSoup) -> bool:
    rendered = False

    for blockquote in list(soup.find_all("blockquote")):
        text = blockquote.get_text("\n", strip=True)
        lines = [line.strip() for line in text.splitlines() if line.strip()]
        if len(lines) == 2 and lines[0].upper() == "[!NOTE]" and _is_issue_number(lines[1]):
            blockquote.replace_with(_make_issue_number_box(soup, lines[1]))
            rendered = True
    ...
    for p in list(soup.find_all("p")):
        if p.find_parent(["li", "blockquote", "td", "th"]):
            continue
        text = p.get_text(" ", strip=True)
        if _is_issue_number(text):
            p.replace_with(_make_issue_number_box(soup, text))
            rendered = True
...
def optimize_for_wechat_html(content_html: str, template: str = "standard") -> str:
    ...
    has_issue_number = _render_issue_number_boxes(soup)

...
description: 从 Markdown 文件或网页链接提取文章并发布到微信公众号。适用于需要“自动创建草稿/提交发布”、统一微信样式（standard/viral）和批量复用发布流程的场景。对少威公众号场景，默认以已确认样文和本地渲染规范作为格式基准。
---
...
- 列举内容是否正确选择轻列举或卡片式列表
- 结尾是否包含居中长分隔线 + 居中 `End`
- 固定尾注是否按确认版分行，而不是挤成一整段

## 执行约束

- 发布前优先做 `--dry-run`，检查标题、摘要和渲染 HTML。
- 对少威公众号场景，不要只检查内容是否通顺，还要检查渲染结构是否符合已确认格式。
